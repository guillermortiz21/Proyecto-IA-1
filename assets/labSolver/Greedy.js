...
class Greedy{
    ...
    getHn(state){
        var Hn;
        if(this.measureType == "Euclidean"){
            console.log("Esta calculando medida euclideana");
            const EuM = Math.pow((this.finalState.row - state.row),2) + Math.pow((this.finalState.column - state.column),2);
            Hn = Math.sqrt(EuM);
        }
        if(this.measureType == "Manhattan"){
            console.log("Esta calculando medida manhattan");
            Hn = Math.abs(state.row - this.finalState.row) + Math.abs(state.column - this.finalState.column);
        }
        return Hn;
    }
